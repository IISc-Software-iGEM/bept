...
# GLOBALS
input_file_name = "7y6i.toml"
TAB_NAMES = ["Input", "Misc-Options", "Output-Settings"]
possible_inputs = ["mg-auto", "mg-para", "mg-manual", "fe-manual", "mg-dummy"]
mg_auto_def = {'read': {'mol': ['pqr', '7y6i.pqr']}, 'elec': {'calculation-type': '', 'dime': ['', '', ''], 'cglen': ['', '', ''], 
                'fglen': ['', '', ''], 'cgcent': ['', ''], 'fgcent': ['', ''], 
                'mol': '1', 'pbe': '', 'bcfl': '', 'pdie': '', 'sdie': '', 'srfm': '', 'chgm': '',
                'sdens': '', 'srad': '', 'swin': '', 'temp': '', 
                'calcenergy': '', 'calcforce': '', 'write': []}, 'print': {'elecEnergy': ['1', 'end']}}
output_file =open("7y6i_out.toml", "w+")
    ...
    def compose(self):
        ...
        with Collapsible(title='mg-auto-options', id='mg-auto-options'):
            ...
            with Collapsible(title="CENTER OF THE COARSE GRID", id="cgcent"):
                with TabbedContent(initial="cgcenter" if self.cgcent[0] == "mol" else "cgmanual"):
                    ...
                    with TabPane("Manually enter coordinates for center of grid", id="cgmanual"):
                        if len(self.cgcent) == 3:
                            cgcent_val_x, cgcent_val_y, cgcent_val_z = self.cgcent
                        else:
                            cgcent_val_x, cgcent_val_y, cgcent_val_z = "", "", ""

                        yield Input(placeholder="x-coordinate", value=cgcent_val_x,type="number",id="cgcent_0")
                        yield Input(placeholder="y-coordinate", value=cgcent_val_y,type="number",id="cgcent_1")
                        yield Input(placeholder="z-coordinate", value=cgcent_val_z,type="number",id="cgcent_2")

            # CENTER OF THE FINE GRID
            with Collapsible(title="CENTER OF THE FINE GRID", id="fgcent"):
                with TabbedContent(initial="fgcenter" if self.fgcent[0] == "mol" else "fgmanual"):
                    # TAB: Center grid on a molecule
                    ...
                    # TAB: Manually enter coordinates for center of grid
                    with TabPane("Manually enter coordinates for center of grid", id="fgmanual"):
                        if len(self.fgcent) == 3:
                            fgcent_val_x, fgcent_val_y, fgcent_val_z = self.fgcent
                        else:
                            fgcent_val_x, fgcent_val_y, fgcent_val_z = "", "", ""

                        yield Input(placeholder="x-coordinate", value=fgcent_val_x,type="number",id="fgcent_0")
                        yield Input(placeholder="y-coordinate", value=fgcent_val_y,type="number",id="fgcent_1")
                        yield Input(placeholder="z-coordinate", value=fgcent_val_z,type="number",id="fgcent_2")

            # TYPE OF PBE TO BE SOLVED
            with Collapsible(title="TYPE OF PBE TO BE SOLVED", id = "pbe"):
                pbe_options = OptionList(Option("Linearised", id = "pbe lpbe"), Option("Non-Linearised", id="pbe npbe"))
            ...
            with Collapsible(title="BOUNDARY CONDITION DEFINITION", id="bcfl"):
                ...
                bcfl_options.highlighted = ["zero", "sdh", "mdh", "focus"].index(self.bcfl)
                yield bcfl_options

            # MOBILE ION SPECIES PRESENT IN SYSTEM (OPTIONAL)
            with Collapsible(title="MOBILE ION SPECIES PRESENT IN SYSTEM (OPTIONAL)"):
                yield Horizontal(
                    Vertical(Label("Charge (Ec)"), Input(placeholder="Ion 1",id="ion_0_0"), Input(placeholder="Ion 2",id="ion_1_0"), Input(placeholder="Ion 3",id="ion_2_0"), classes="input-boxes"),
                    Vertical(Label("Concentration (M)"), Input(placeholder="Ion 1",id="ion_0_1"), Input(placeholder="Ion 2",id="ion_1_1"), Input(placeholder="Ion 3",id="ion_2_1"), classes="input-boxes"),
                    Vertical(Label("Radius A"), Input(placeholder="Ion 1",id="ion_0_2"), Input(placeholder="Ion 2",id="ion_1_2"), Input(placeholder="Ion 3",id="ion_2_2"), classes="input-boxes"),
                    classes="input-container"
                )
            # BIOMOLECULAR DIELECTRIC CONSTANT
            ...
            # METHOD BY WHICH THE BIOMOLECULAR POINT CHARGES ARE MAPPED ONTO THE GRID
            with Collapsible(title="METHOD BY WHICH THE BIOMOLECULAR POINT CHARGES ARE MAPPED ONTO THE GRID", id="chgm"):
            ...
            # NUMBER OF GRID POINTS PER SQUARE-ANGSTROM TO USE IN SURFACE CONSTRUCTIONS
            with Collapsible(title="NUMBER OF GRID POINTS PER SQUARE-ANGSTROM TO USE IN SURFACE CONSTRUCTIONS", id="sdens"):
                yield Input(value=self.sdens, type="number", id="sdens")

            # MODEL TO USE TO CONSTRUCT THE DIELECTRIC ION-ACCESSIBILITY COEFFICIENTS
            with Collapsible(title="MODEL TO USE TO CONSTRUCT THE DIELECTRIC ION-ACCESSIBILITY COEFFICIENTS", id="srfm"):
                ...
                srfm_options.highlighted = ["mol", "smol", "spl2", "spl4"].index(self.srfm)
                yield srfm_options
            ...
            # SIZE OF THE SUPPORT FOR SPLINE-BASED SURFACE DEFINITIONS
            with Collapsible(title="SIZE OF THE SUPPORT FOR SPLINE-BASED SURFACE DEFINITIONS",id="swin"):
...
class Mg_para_options(Static):
    ...
    def compose(self):
        ...
        with Collapsible(title='mg-para-options', id='mg-para-options'):
            ...
            # AMOUNT OF OVERLAP TO INCLUDE BETWEEN THE INDIVIDUAL PROCESSORS' MESHES
            with Collapsible(title="AMOUNT OF OVERLAP TO INCLUDE BETWEEN THE INDIVIDUAL PROCESSORS' MESHES", id="ofrac"):
...
class Mg_manual_options(Static):
    ...
    def compose(self):
        ...
        with Collapsible(title='mg-manual-options', id='mg-manual-options'):
            ...
            with Collapsible(title="DEPTH OF THE MULTILEVEL HIERARCHY USED IN THE MULTIGRID SOLVER"):
                yield Input(value="4",id="nlev",type="number")

            # CENTER OF THE GRID
            with Collapsible(title="CENTER OF THE GRID", id="gcent"):
                with TabbedContent(initial="gcenter" if self.gcent[0] == "mol" else "gmanual"):
                    # TAB: Center grid on a molecule
                    ...
                    # TAB: Manually enter coordinates for center of grid
                    with TabPane("Manually enter coordinates for center of grid", id="gmanual"):
                        if len(self.gcent) == 3:
                            gcent_val_x, gcent_val_y, gcent_val_z = self.gcent
                        else:
                            gcent_val_x, gcent_val_y, gcent_val_z = "", "", ""

                        yield Input(placeholder="x-coordinate", value=gcent_val_x,id="gcent_0",type="number")
                        yield Input(placeholder="y-coordinate", value=gcent_val_y,id="gcent_1",type="number")
                        yield Input(placeholder="z-coordinate", value=gcent_val_z,id="gcent_2",type="number")
...
class Misc_options(Static):
    ...
    def compose(self):
        ...
        with VerticalScroll():
            ...
            with Collapsible(title="CALCULATION OF ELECTROSTATIC ENERGY FROM A PBE CALCULATION:", id="calcenergy"):
                with RadioSet(id="energy"):
                    yield RadioButton("Don't calculate any energies", id="no", value=(self.calcforce == "no"))
                    yield RadioButton("Calculate and return total electrostatic energy for the entire molecule ", id="total", value=(self.calcforce == "total"))
                    yield RadioButton("Calculate and return total electrostatic energy for the entire molecule as well as energy components for each atom ", id="comps", value=(self.calcforce == "comps"))

            # Force Calculations
            yield Label("Force Calculations")
            with Collapsible(title="CALCULATION OF ELECTROSTATIC AND APOLAR FORCE OUTPUTS FROM A PBE CALCULATION:", id="calcforce"):
                with RadioSet(id="force"):
                    yield RadioButton("Don't calculate any forces", id="no", value=(self.calcenergy == "no"))
                    yield RadioButton("Calculate and return total electrostatic and apolar forces for the entire molecule", id="total", value=(self.calcenergy == "total"))
                    yield RadioButton("Calculate and return total electrostatic and apolar forces for the entire molecule as well as force components for each atom", id="comps", value=(self.calcenergy == "comps"))
...
class Output_options(Static):
    ...
    def compose(self):
        ...
        with VerticalScroll():
            ...
            with Collapsible(title="OUTPUT OF SCALAR DATA CALCULATED DURING THE PB RUN:"):
                yield Checkbox("Write out the biomolecular charge distribution in units of ec (multigrid only)", id="charge", value=("write-charge" in data["elec"]))
                yield Checkbox("Write out the electrostatic potential in units of kbT/ec (multigrid and finite element)", id="pot", value=("write-pot" in data["elec"]))
                yield Checkbox("Write out the solvent accessibility defined by the molecular surface definition", id= "smol", value=("write-smol" in data["elec"]))
                yield Checkbox("Write out the spline-based solvent accessibility", id="sspl", value=("write-sspl" in data["elec"]))
                yield Checkbox("Write out the van der Waals-based solvent accessibility", id="vdw", value=("write-vdw" in data["elec"]))
                yield Checkbox("Write out the inflated van der Waals-based ion accessibility", id="ivdw", value=("write-ivdw" in data["elec"]))
                yield Checkbox("Write out the Laplacian of the potential in units of kBT/ec/A2 (multigrid only)", id="lap", value=("write-lap" in data["elec"]))
                yield Checkbox("Write out the \"energy density\" in units of kBT/ec/A2 (multigrid only)", id="edens", value=("write-edens" in data["elec"]))
                yield Checkbox("Write out the mobile ion number density for m ion species in units of M (multigrid only)", id="ndens", value=("write-ndens" in data["elec"]))
                yield Checkbox("Write out the mobile charge density for m ion species in units of eₜᶜ M (multigrid only)", id="qdens", value=("write-qdens" in data["elec"]))
                yield Checkbox("Write out the dielectric map shifted by 1/2 grid spacing in the x-direction", id="dielx", value=("write-dielx" in data["elec"]))
                yield Checkbox("Write out the dielectric map shifted by 1/2 grid spacing in the y-direction", id="diely", value=("write-diely" in data["elec"]))
                yield Checkbox("Write out the dielectric map shifted by 1/2 grid spacing in the z-direction", id="dielz", value=("write-dielz" in data["elec"]))
            ...
            with Collapsible(title="FORMAT TO WRITE DATA:"):
                with RadioSet(id="format"):
                    yield RadioButton("OpenDX", id="dx", value=(self.write_pot[0] == "dx"))
                    yield RadioButton("AVS UCD", id="avs", disabled=(InputApp.selected_input != "fe-manual"), value=(self.write_pot[0] == "avs"))
                    yield RadioButton("UBHD", id="uhbd", disabled=(InputApp.selected_input == "fe-manual"), value=(self.write_pot[0] == "uhbd"))
...
class InputApp(App):
        ...
        with TabbedContent(initial="Input"):
            for tab in TAB_NAMES:
                with TabPane(tab, id=tab):
                    if tab == "Input":
                        yield Select([(inp, inp) for inp in possible_inputs], allow_blank=False, value=self.selected_input).focus()
                    else:
    ...
    def on_select_changed(self, event: Select.Changed):
        ...
        self.selected_input = event.value
        self.new_data["elec"]["calculation-type"] = self.selected_input

        # Adding the mg-auto options list
        if self.selected_input == "mg-auto" :
        ...
        elif self.selected_input == "mg-para":
            self.query(Collapsible).remove()
        ...
        elif self.selected_input == "fe-manual":
            self.query(Collapsible).remove()
        ...
        elif self.selected_input == "mg-manual":
            self.query(Collapsible).remove()
        ...
        elif self.selected_input == "mg-dummy":
            self.query(Collapsible).remove()
    ...
    def on_radio_set_changed(self, event: RadioSet.Changed):
        ...
        if str(event.radio_set.id) == "energy":
            self.calcenergy = event.pressed.id
        elif str(event.radio_set.id) == "force":
            self.calcforce = event.pressed.id
        elif str(event.radio_set.id) == "format":
            self.form = event.pressed.id

    @on(Checkbox.Changed)
    def on_checkbox_changed(self, event:Checkbox.Changed):
        """Triggers when the Checkbox widget is changed"""
        if event.value == True:
            self.write_commands.append([event.checkbox.id])
    ...
    def on_input_submit(self, event: Input.Changed):
        ...
        if "gcent" in event.input.id:
            ...
            self.ion[int(event.input.id.split("_")[1])][int(event.input.id.split("_")[2])] = event.value
            if '' not in self.ion[int(event.input.id.split("_")[1])]:
                ...
                if "ion" in list(self.new_data['elec'].keys()):
                    ...
                    item.insert(n,("ion",[self.ion[int(event.input.id.split("_")[1])]]))
                self.new_data['elec'] = dict(item)
        elif "_" in event.input.id:
            self.new_data["elec"][event.input.id.split('_')[0]][int(event.input.id.split('_')[1])] = event.value
        else:
    ...
    def action_quit(self) -> Coroutine[Any, Any, None]:
        ...
        self.new_data["elec"]["calcforce"] = self.calcforce
        self.new_data["elec"]["calcenergy"] = self.calcenergy
        for i in range(len(self.write_commands)):
            self.write_commands[i].append(self.form)
            self.write_commands[i].append(data["read"]["mol"][1].split(".")[0] + ".pqr")
        self.new_data["elec"]["write"] = self.write_commands
...
for i in InputApp.ion:
    if i.count("") == 2 or i.count("") == 1:
        warnings.warn("All the parameters for ion"+ str(InputApp.ion.index(i)+1)+ " are not provided, and it will be ignored in the calculation")
